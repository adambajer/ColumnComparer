...
def read_table(path):
    ...
    raise RuntimeError(f"Nepodporovaný formát: {ext}")
...
class ColumnComparerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        lf = ttk.Frame(left)
        lf.pack(fill=tk.X)
        ttk.Button(lf, text="Načíst tabulku 1", command=self.load_table1).pack(side=tk.LEFT)
        ...
        rf = ttk.Frame(right)
        rf.pack(fill=tk.X)
        ttk.Button(rf, text="Načíst tabulku 2", command=self.load_table2).pack(side=tk.LEFT)
        ...
        ttk.Button(bottom, text="Porovnat", command=self.compare).pack(side=tk.LEFT, padx=10)
        ttk.Button(bottom, text="Export výsledku", command=self.export).pack(side=tk.LEFT, padx=10)
    ...
    def _create_tree(self, parent, header_callback):
        ...
        # kliknutí na hlavičku
        if header_callback:
            def handler(event, tv=tree, cb=header_callback):
                reg = tv.identify_region(event.x, event.y)
                if reg != "heading":
                    return
                cid = tv.identify_column(event.x)
                idx = int(cid[1:]) - 1
    ...
    def fill_tree(self, tree, headers, rows):
        ...
        for row in rows:
            tree.insert("", tk.END, values=[row[h] for h in headers])
    ...
    def compare(self):
        if not self.col1 or not self.col2:
            messagebox.showwarning("Chyba", "Vyber oba sloupce kliknutím na hlavičky.")
            return
        ...
        messagebox.showinfo("Hotovo", f"Nalezeno {len(self.result_rows)} shod.")

    # ------------------------------------------------------------------
    def export(self):
        if not self.result_rows:
            messagebox.showwarning("Nic k exportu", "Není žádný výsledek.")
            return
